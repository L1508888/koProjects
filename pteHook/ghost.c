...
/*
 * We only touch vm_start/vm_end of a VMA, which are the first two fields on
 * all supported kernels; alias to a minimal struct so we don't depend on the
 * full (version-specific) vm_area_struct layout.
 */
 struct vma_head {
...
void __nocfi ghost_mm_init(const struct ghost_mm_syms *syms)
{
    g_syms = *syms;
}
 ...
 static int pte_occupied_cb(pte_t *pte, unsigned long addr, void *data)
 {
 ...
 static __nocfi int vaddr_is_occupied(struct mm_struct *mm, unsigned long va)
 {
     int occupied = 0;
 ...
 int __nocfi ghost_alloc(struct task_struct *task,
                 struct mm_struct   *mm,
                 unsigned long       near,
                 unsigned long       range,
                 ...
                 struct ghost_page  *out)
 {
     ...
     kva = g_syms.get_free_pages(GFP_KERNEL | __GFP_ZERO, order);
 ...
 int __nocfi ghost_free(struct ghost_page *gp)
 {
     int i;
     int page_count;
 ...
 int __nocfi ghost_write(struct ghost_page *gp,
                 unsigned offset,
                 const void *src,
                 unsigned len)
 {
 ...
 void __nocfi ghost_sync_icache(struct ghost_page *gp)
 {
